...
save_path = f'{settings.path}/training'+'/level_three/'
...
LK_layer_three = Lucas_Kanade_layer(batch_size=batch_size,height_template=32,width_template=32,num_channels=1)

dfhistory = pd.DataFrame(columns=['epoch', 'convex_loss', 'equ1_loss', 'equ2_loss', 'equ3_loss', 'ssim_loss', 'total_loss'])
for current_epoch in range(settings.train_stage3):
    ...
    for iters in range(100000000):
        ...
        if len(np.shape(input_img))<2:
          level_three_input.save_weights(save_path +'epoch_'+str(1+current_epoch)+"input_full")
          level_three_template.save_weights(save_path +'epoch_'+str(1+current_epoch)+"template_full")
          break
        ...
        gt_matrix_three = construct_matrix(gt_matrix,scale_factor=0.25,batch_size=batch_size)
        ...
        with tf.GradientTape() as tape:
            ...
            for nn in range(settings.sample_noise):
                ...
                noise_three = gt_motion_rs_random_noisy(u_list,v_list,batch_size=batch_size,lambda_noisy=lambda_three)

                lambda_noisy_labels_three_plus =LK_layer_three.projective_inverse_warp(input_feature_map_three, gt_matrix_three + rho * noise_three)
                h_lambda_noisy_labels_three_plus = tf.reduce_mean(compute_ssim(template_feature_map_three, lambda_noisy_labels_three_plus))

                lambda_noisy_labels_three_mins = LK_layer_three.projective_inverse_warp(input_feature_map_three, gt_matrix_three - rho * noise_three)
                h_lambda_noisy_labels_three_mins = tf.reduce_mean(compute_ssim(template_feature_map_three,lambda_noisy_labels_three_mins))

                noisy_labels_three_plus = LK_layer_three.projective_inverse_warp(input_feature_map_three, gt_matrix_three + noise_three)
                h_noisy_labels_three_plus = tf.reduce_mean(compute_ssim(template_feature_map_three,noisy_labels_three_plus))

                noisy_labels_three_mins = LK_layer_three.projective_inverse_warp(input_feature_map_three, gt_matrix_three - noise_three)
                h_noisy_labels_three_mins = tf.reduce_mean(compute_ssim(template_feature_map_three,noisy_labels_three_mins))
                
                if settings.option == 'option1':
                    # equ1
                    equ1_plus= tf.math.maximum(ssim_middle_three - h_lambda_noisy_labels_three_plus + mu * (rho**2) * np.sum(lambda_three**2) / 2 ,0)
                    equ1_mins= tf.math.maximum(ssim_middle_three - h_lambda_noisy_labels_three_mins + mu * (rho**2) * np.sum(lambda_three**2) / 2 ,0)
                elif settings.option == 'option2':
                    # equ1
                    equ1_plus= tf.math.maximum(ssim_middle_three - h_lambda_noisy_labels_three_plus, 0)
                    equ1_mins= tf.math.maximum(ssim_middle_three - h_lambda_noisy_labels_three_mins, 0)
                # equ2
                equ2_plus = tf.math.maximum(ssim_middle_three + mu * np.sum(lambda_three**2) / 2 - h_noisy_labels_three_plus,0)
                equ2_mins = tf.math.maximum(ssim_middle_three + mu * np.sum(lambda_three**2) / 2 - h_noisy_labels_three_mins,0)
                # equ3
                equ3_plus = tf.math.maximum(h_lambda_noisy_labels_three_plus - (1 - rho) * ssim_middle_three - rho * h_noisy_labels_three_plus + mu * rho * (1 - rho) * np.sum(lambda_three**2) / 2 ,0)
                equ3_mins = tf.math.maximum(h_lambda_noisy_labels_three_mins - (1 - rho) * ssim_middle_three - rho * h_noisy_labels_three_mins + mu * rho * (1 - rho) * np.sum(lambda_three**2) / 2 ,0)
    ...
    dfhistory.to_csv(f'{settings.path}/training/dfhistory_level3.csv', index=False)
    ...
    level_three_input.save_weights(save_path +'epoch_'+str(1+current_epoch)+"input_"+str(iters))
    level_three_template.save_weights(save_path +'epoch_'+str(1+current_epoch)+"template_"+str(iters))
